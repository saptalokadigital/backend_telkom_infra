...
// Add cable to cart
exports.addToCartCable = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    const cableId = req.body.cableId;
...
exports.getCartCables = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    if (!token)
...
exports.removeFromCartCable = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    ...
    jwt.verify(token, process.env.JWT_KEY, async function (err, decoded) {
      ...
      await user.save();
      return res.status(200).send({ message: "Cable removed from cart." });
    });
  } catch (error) {
    return res.status(500).send({
      auth: false,
...
exports.deleteAllCartCables = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    if (!token)
...
exports.addToCartKit = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
...
exports.getCartKits = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    if (!token)
...
exports.removeFromCartKit = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
...
exports.deleteAllCartKits = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    if (!token)
...
exports.getTurnOverCableFromCart = async (req, res) => {
  try {
    const authHeader = req.headers["authorization"];
    const token = authHeader && authHeader.split(" ")[1];
    if (!token)
    ...
    jwt.verify(token, process.env.JWT_KEY, async function (err, decoded) {
      ...
      const uniqueTurnOver = turnOver.flat().reduce((acc, cable) => {
        if (!cartCables.some((cartCable) => cartCable.id === cable.id)) {
