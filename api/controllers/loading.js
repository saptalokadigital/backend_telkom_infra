...
exports.editKitInLoading = async (req, res, next) => {
  ...
  if (kits_id > 0) {
    const isValidIds = comments.every((kits_id) => mongoose.Types.ObjectId.isValid(kits_id));
    if (!isValidIds) {
      return res.status(400).json({ error: "Invalid comment IDs" });
    }
  }
...
exports.getLoadingById = async (req, res, next) => {
  ...
  try {
    ...
    const loading = await loadingModel.findById(loadingId).populate("cables_id");
...
exports.postLoading = async (req, res, next) => {
  try {
    ...
    return res.status(200).send({ result, msg: "Added new loading succesfully", sukses: true });
  } catch (error) {
    return res.status(500).send({
      msg: "Ooopps, Failed to added new loading",
      sukses: false,
    });
  }
};
  ...
  new Promise((resolve, reject) => {
    loadingModel
      ...
      .catch(() =>
        reject({
          sukses: false,
          msg: "Ooopps, Failed to get loading data",
          data: [],
        })
      );
  });
